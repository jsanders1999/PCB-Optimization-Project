...
def TestReadAndOptPCB_Loop(N2, M2, res, tar_dir, matrixGenerator = P_AssembleS, solutionplots = False):
    ...
    for N1 in range(1,min(13,N2+1)):
        ...
        if tar_dir == "x":
            ...
            r_ls = np.linalg.norm(np.ones((res**3)) - Sx@u_ls)**2/(res**3)
            ...
            #u_ls = sp.linalg.solve(L.T, y)#, assume_a = "pos")
            r_ls = np.linalg.norm(np.ones((res**3)) - Sz@u_ls)**2/(res**3)
        ...
        #Plotting
        if solutionplots: #and N1%2 == 0:
            
            #fig3, ax3 = PlotSol(u_ls, N1, M1, res, r_ls, B_ls, tar_dir)
            #fig3, ax3 = PlotSol3D(u_ls, Sx, Sy, Sz, tar_dir, N, M, res)
            contours, I = CalcContoursHarmonics(u_ls, tar_dir, N1, M1)
            Bx, By, Bz, U = CalcUniformityContours(contours, res, I)
            Ulist += [U]
        

    
    return solutionlist, Vlist, Blist, Plist, Ulist
...
def PlotMetrics(ulists, Vlists, Blists, Plists, dirs = ["x", "z"], inds = [0, 2]):
    ...
    for i, Blist in enumerate(Blists):
        ax5[1].plot(range(1+offset[dirs[i]], 2*len(Blist)+1+offset[dirs[i]], 2), np.array(Blist)[:,inds[i]],  marker = ".", linestyle = "--", label = "{} target".format(dirs[i]))
    ...
    #ax5.set_ylim((0, 1.2*max(Vlist)))
    ax5[1].set_xlabel("Number of harmonics")
    ax5[1].set_ylabel("Average B field")
    ax5[1].legend()
    ax5[1].grid()


    ax5[2].set_xlabel("Number of harmonics")
    ax5[2].set_ylabel("Power dissapated")
...
def CalcContoursHarmonics(u, tar_dir, N, M, seperation = 0.0025):
    if tar_dir=="z":
        HarmStreamFunc = HarmStreamFunc_Ztar
    elif tar_dir == "x":
        HarmStreamFunc = HarmStreamFunc_Xtar
...
def CalcUniformityContours(contours, resolution, I, V = [-0.5, 0.5, -0.5, 0.5, 0.1, 1.1]):
    ...
    U = (np.sum((Bx)**2) + np.sum((By)**2) + np.sum((Bz-1)**2))/(res**3*1**2)
    print("U lines: ", U)
...
if __name__ == "__main__":
    ...
    res = 10#22 #Must be even when using the symmetric matric constructors!
    N = 7#18
    M = N
    #u, V = TestGenAndOptPCB(N, M, res, tar_dir = "x", matrixGenerator = P_AssembleSSymRightOrder)
    #ulist_x, Vlist_x, Blist_x, Plist_x = TestReadAndOptPCB_Loop(N, M, res, tar_dir = "x", matrixGenerator = P_AssembleSSymRightOrder, solutionplots=True)
    ulist_z, Vlist_z, Blist_z, Plist_z, Ulist_z = TestReadAndOptPCB_Loop(N, M, res, tar_dir = "z", matrixGenerator = P_AssembleSSymRightOrder, solutionplots=True)
